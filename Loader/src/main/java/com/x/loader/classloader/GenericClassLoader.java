...
package com.x.loader.classloader;

import com.x.loader.classloader.resources.resource.core.IResource;
import com.x.loader.classloader.resources.loader.core.IResourceLoader;
...
public class GenericClassLoader extends URLClassLoader {
    ...
    protected final IResourceLoaderFactory resourceLoaderFactory;

    private final Map<String, Class<?>> pluginClassCache = new ConcurrentHashMap<>();

    public GenericClassLoader(String name, IResourceLoaderFactory resourceLoaderFactory) {
        this(name, null, resourceLoaderFactory);
    }

    public GenericClassLoader(String name, ClassLoader parent, IResourceLoaderFactory resourceLoaderFactory) {
        super(new URL[]{}, null);
    ...
    protected Class<?> findClassFromLocal(String name) {
        ...
        IResource resource = resourceLoaderFactory.findResource(formatClassName);
        byte[] bytes = null;
        if(resource != null){
    ...
    protected URL findResourceFromLocal(String name) {
        IResource resource = resourceLoaderFactory.findResource(name);
        if (resource == null) {
            return null;
        }
    ...
    protected Enumeration<URL> findResourcesFromLocal(String name) throws IOException{
        Enumeration<IResource> enumeration = resourceLoaderFactory.findResources(name);
        return new Enumeration<URL>() {
            @Override
            public boolean hasMoreElements() {
