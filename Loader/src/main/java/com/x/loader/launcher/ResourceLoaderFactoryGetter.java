...
import com.x.loader.classloader.resources.loader.core.DefaultResourceLoaderFactory;
import com.x.loader.classloader.resources.loader.core.IResourceLoaderFactory;
import com.x.loader.classloader.resources.resource.CacheResourceManager;
import com.x.loader.classloader.resources.resource.ShareResourceManager;
...
public class ResourceLoaderFactoryGetter {
    ...
    static IResourceLoaderFactory get(String classLoaderName, String... args){
        if(resourceMode == null){
            synchronized (IResourceLoaderFactory.class){
                if(resourceMode == null){
    ...
    public static BaseResourceManager getResourceManager(String key, URL baseUrl){
        BaseResourceManager resourceStorage = null;
        if(Objects.equals(resourceMode, RESOURCE_MODE_NO_CACHE)){
            resourceStorage = new DefaultResourceManager(baseUrl);
        } else if(Objects.equals(resourceMode, RESOURCE_MODE_CACHE_SHARE)){
            resourceStorage = new ShareResourceManager(key, baseUrl);
        } else {
            resourceStorage = new CacheResourceManager(baseUrl);
        }
