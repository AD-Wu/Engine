...
public class Handler extends URLStreamHandler {
    ...
    private static SoftReference<Map<File, XJarFile>> rootFileCache;
    ...
    protected URLConnection openConnection(URL url) throws IOException {
        ...
        if (this.jarFile != null && isUrlInJarFile(url, this.jarFile)) {
            jarURLConnection = XJarURLConnection.get(url, this.jarFile);
        } else {
            try {
                jarURLConnection = XJarURLConnection.get(url, getRootJarFileFromUrl(url));
            } catch (Exception ex) {
    ...
    private boolean isUrlInJarFile(URL url, XJarFile jarFile) throws MalformedURLException {
    ...
    public XJarFile getRootJarFileFromUrl(URL url) throws IOException {
    ...
    private XJarFile getRootJarFile(String name) throws IOException {
        try {
            if (!name.startsWith(FILE_PROTOCOL)) {
                throw new IllegalStateException("Not a file URL");
            }
            File file = new File(URI.create(name));
            XJarFile jarFile = PluginResourceManager.getRootJarFile(file);
            if (jarFile == null) {
                jarFile = new XJarFile(file);
                PluginResourceManager.addRootJarFile(file, jarFile);
            }
            return jarFile;
        } catch (Exception ex) {
    ...
    /**
     * Add the given {@link XJarFile} to the root file cache.
     * @param sourceFile the source file to add
     * @param jarFile the jar file.
     */
    static void addToRootFileCache(File sourceFile, XJarFile jarFile) {
        Map<File, XJarFile> cache = rootFileCache.get();
        if (cache == null) {
        ...
        if (canResetCachedUrlHandlers()) {
            ...
            try {
                ...
                try {
                    ...
                    if (connection instanceof XJarURLConnection) {
                        jarContextUrl = null;
                    }
                } catch (Exception ex) {
                    // ignore
    ...
    public static void setUseFastConnectionExceptions(boolean useFastConnectionExceptions) {
        XJarURLConnection.setUseFastExceptions(useFastConnectionExceptions);
    }
