...
import com.x.loader.PluginResourceManager;
...
public class XJarURLConnection extends JarURLConnection {
    ...
    private static final XJarURLConnection NOT_FOUND_CONNECTION = XJarURLConnection.notFound();
    ...
    private XJarURLConnection(URL url, AbstractJarFile jarFile, JarEntryName jarEntryName) throws IOException {
    ...
    public synchronized InputStream getInputStream() throws IOException {
        ...
        if (this.jarEntryName.isEmpty() && this.jarFile.getType() == XJarFile.JarFileType.DIRECT) {
    ...
    static void setUseFastExceptions(boolean useFastExceptions) {
        XJarURLConnection.useFastExceptions.set(useFastExceptions);
    }

    static XJarURLConnection get(URL url, XJarFile jarFile) throws IOException {
        ...
        if (index == -1) {
            return (Boolean.TRUE.equals(useFastExceptions.get()) ? NOT_FOUND_CONNECTION
                    : new XJarURLConnection(url, null, EMPTY_JAR_ENTRY_NAME));
        }
        int separator;
        while ((separator = spec.indexOf(SEPARATOR, index)) > 0) {
            JarEntryName entryName = JarEntryName.get(spec.subSequence(index, separator));
            XJarEntry jarEntry = jarFile.getJarEntry(entryName.toCharSequence());
            if (jarEntry == null) {
                return XJarURLConnection.notFound(jarFile, entryName);
            }
        ...
        PluginResourceManager.addJarFile(jarFileWrapper);
        return new XJarURLConnection(url, jarFileWrapper, jarEntryName);
    }
    ...
    private static XJarURLConnection notFound() {
        try {
    ...
    private static XJarURLConnection notFound(XJarFile jarFile, JarEntryName jarEntryName) throws IOException {
        if (Boolean.TRUE.equals(useFastExceptions.get())) {
            return NOT_FOUND_CONNECTION;
        }
        return new XJarURLConnection(null, jarFile, jarEntryName);
    }
