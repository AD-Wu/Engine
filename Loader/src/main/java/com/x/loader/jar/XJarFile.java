 ...
 * @version 3.0.0
 */
public class XJarFile extends AbstractJarFile implements Iterable<JarEntry> {
    ...
    // private static final String HANDLERS_PACKAGE = "com.gitee.starblues.loader";

    private static final String HANDLERS_PACKAGE = "com.x.loader";
    ...
    private final IRandomAccessData data;
    ...
    /**
     * Create a new {@link XJarFile} backed by the specified file.
     ...
     * @throws IOException if the file cannot be read
     */
    XJarFile(RandomAccessDataFile file) throws IOException {
        this(file, "", file, JarFileType.DIRECT);
    }

    /**
     * Private constructor used to create a new {@link XJarFile} either directly or from a
     * nested entry.
     ...
     * @throws IOException if the file cannot be read
     */
    private XJarFile(RandomAccessDataFile rootFile, String pathFromRoot, IRandomAccessData data, JarFileType type)
            throws IOException {
        this(rootFile, pathFromRoot, data, null, type, null);
    }

    private XJarFile(RandomAccessDataFile rootFile, String pathFromRoot, IRandomAccessData data, IJarEntryFilter filter,
    ...
    private ICentralDirectoryVisitor centralDirectoryVisitor() {
        return new ICentralDirectoryVisitor() {

            @Override
            public void visitStart(CentralDirectoryEndRecord endRecord, IRandomAccessData centralDirectoryData) {
                XJarFile.this.comment = endRecord.getComment();
            }
    ...
    public XJarEntry getJarEntry(CharSequence name) {
        return this.entries.getEntry(name);
    }

    @Override
    public XJarEntry getJarEntry(String name) {
        return (XJarEntry) getEntry(name);
    }
    ...
    public synchronized InputStream getInputStream(ZipEntry entry) throws IOException {
        ensureOpen();
        if (entry instanceof XJarEntry) {
            return this.entries.getInputStream((XJarEntry) entry);
        }
    ...
    /**
     * Return a nested {@link XJarFile} loaded from the specified entry.
     * @param entry the zip entry
     * @return a {@link XJarFile} for the entry
     * @throws IOException if the nested jar file cannot be read
     */
    public synchronized XJarFile getNestedJarFile(ZipEntry entry) throws IOException {
        return getNestedJarFile((XJarEntry) entry);
    }
     ...
     * @throws IOException if the nested jar file cannot be read
     */
    public synchronized XJarFile getNestedJarFile(XJarEntry entry) throws IOException {
        try {
    ...
    private XJarFile createJarFileFromEntry(XJarEntry entry) throws IOException {
    ...
    private XJarFile createJarFileFromDirectoryEntry(XJarEntry entry) throws IOException {
        AsciiBytes name = entry.getAsciiBytesName();
        IJarEntryFilter filter = (candidate) -> {
            if (candidate.startsWith(name) && !candidate.equals(name)) {
                return candidate.substring(name.length());
            }
            return null;
        };
        return new XJarFile(this.rootFile, this.pathFromRoot + "!/" + entry.getName().substring(0, name.length() - 1),
                            this.data, filter, JarFileType.NESTED_DIRECTORY, this.manifestSupplier);
    }

    private XJarFile createJarFileFromFileEntry(XJarEntry entry) throws IOException {
        ...
        IRandomAccessData entryData = this.entries.getEntryData(entry.getName());
        return new XJarFile(this.rootFile, this.pathFromRoot + "!/" + entry.getName(), entryData,
    ...
    JarEntryCertification getCertification(XJarEntry entry) {
        try {
    ...
    private static class JarEntryEnumeration implements Enumeration<java.util.jar.JarEntry> {
        ...
        private final Iterator<XJarEntry> iterator;

        JarEntryEnumeration(Iterator<XJarEntry> iterator) {
