 ...
 * @version 3.0.0
 */
public class JarFileEntries implements ICentralDirectoryVisitor, Iterable<XJarEntry> {
    ...
    private final IJarEntryFilter filter;

    private IRandomAccessData centralDirectoryData;
    ...
    private final Map<Integer, IFileHeader> entriesCache = Collections
            .synchronizedMap(new LinkedHashMap<Integer, IFileHeader>(16, 0.75f, true) {

                @Override
                protected boolean removeEldestEntry(Map.Entry<Integer, IFileHeader> eldest) {
                    return size() >= ENTRY_CACHE_SIZE;
                }

            });

    JarFileEntries(XJarFile jarFile, IJarEntryFilter filter) {
    ...
    @Override
    public Iterator<XJarEntry> iterator() {
        return new EntryIterator(NO_VALIDATION);
    }

    Iterator<XJarEntry> iterator(Runnable validator) {
        return new EntryIterator(validator);
    }

    boolean containsEntry(CharSequence name) {
        return getEntry(name, IFileHeader.class, true) != null;
    }

    XJarEntry getEntry(CharSequence name) {
        return getEntry(name, XJarEntry.class, true);
    }

    InputStream getInputStream(String name) throws IOException {
        IFileHeader entry = getEntry(name, IFileHeader.class, false);
        return getInputStream(entry);
    }

    InputStream getInputStream(IFileHeader entry) throws IOException {
        if (entry == null) {
            return null;
        }
    ...
    IRandomAccessData getEntryData(String name) throws IOException {
        IFileHeader entry = getEntry(name, IFileHeader.class, false);
        if (entry == null) {
            return null;
        }
    ...
    private IRandomAccessData getEntryData(IFileHeader entry) throws IOException {
        // aspectjrt-1.7.4.jar has a different ext bytes length in the
        // local directory to the central directory. We need to re-read
        // here to skip them
        IRandomAccessData data = this.jarFile.getData();
    ...
    private <T extends IFileHeader> T getEntry(CharSequence name, Class<T> type, boolean cacheEntry) {
        T entry = doGetEntry(name, type, cacheEntry, null);
        if (!isMetaInfEntry(name) && isMultiReleaseJar()) {
            int version = RUNTIME_VERSION;
            AsciiBytes nameAlias = (entry instanceof XJarEntry) ? ((XJarEntry) entry).getAsciiBytesName()
    ...
    private <T extends IFileHeader> T doGetEntry(CharSequence name, Class<T> type, boolean cacheEntry,
    ...
    private <T extends IFileHeader> T getEntry(int hashCode, CharSequence name, char suffix, Class<T> type,
    ...
    private <T extends IFileHeader> T getEntry(int index, Class<T> type, boolean cacheEntry, AsciiBytes nameAlias) {
        try {
            IFileHeader cached = this.entriesCache.get(index);
            IFileHeader entry = (cached != null) ? cached : CentralDirectoryFileHeader
                    .fromRandomAccessData(this.centralDirectoryData, this.centralDirectoryOffsets[index], this.filter);
            if (CentralDirectoryFileHeader.class.equals(entry.getClass()) && type.equals(XJarEntry.class)) {
                entry = new XJarEntry(this.jarFile, index, (CentralDirectoryFileHeader) entry, nameAlias);
            }
    ...
    JarEntryCertification getCertification(XJarEntry entry) throws IOException {
    ...
    private int getEntryIndex(CharSequence name) {
        ...
        int index = getFirstIndex(hashCode);
        while (index >= 0 && index < this.size && this.hashCodes[index] == hashCode) {
            IFileHeader candidate = getEntry(index, IFileHeader.class, false, null);
     ...
     */
    private final class EntryIterator implements Iterator<XJarEntry> {
        ...
        public XJarEntry next() {
            ...
            this.index++;
            return getEntry(entryIndex, XJarEntry.class, false, null);
        }
